...
const ADMIN_ID = 5645578129;
const BOT_TOKEN = "8767429586:AAG89PwWd2bx5IzabNeX8qXosUhD9Lph6Es";
...
let requiredChannels = [
  { id: "@minimyid", name: "Mini, {myid),TRX,,CHANNEL🏆🏆" },
  { id: "@minimyid_chat", name: "Mini, {myid),CHAT, GP" }
];
...
const generateProfitTargetAscii = (startedAmount, totalProfit, finalBalance) => {
  return `📢 အနိင်ရရှိပါသည်       

𝑺𝑻𝑨𝑹𝑻𝑬𝑫 𝑨𝑴𝑶𝑼𝑵𝑻:  ${startedAmount.toFixed(2).padStart(12)} Ks
𝑻𝑶𝑻𝑨𝑳 :  +${totalProfit.toFixed(2).padStart(11)} Ks
...
const makeMainKeyboard = (loggedIn = false, userId = null) => {
  ...
  return {
    keyboard: [
      ["🔋 ကစားမည်", "🪫 ရပ်မည်"],
      ["💊 ထိုးကြေး","📟 ထိန်းချုပ်မူ့"],
      ["🎃 နေရာသတ်မှတ်ရန်","🗃 အချက်အလက်"],
      ["🔐 အကောင့်ပြန်၀င်မယ်"]
    ],
    resize_keyboard: true,
    one_time_keyboard: false
...
🎯 𝑮𝒂𝒎𝒆: ${gameType}
🧩 𝑪𝒖𝒓𝒓𝒆𝒏𝒕 လက်ကျန်ငွေ: ${(currentBalance ).toFixed(2)} Ks
...
const formatLoginMessage = (platform, gameName) => {
  ...
  return `မင်္ဂလာပါ Ck Lottery ကို အော်တို ကစား ပေးသော Bot မှာ ကြိုဆိုပါသည် စတင်ကစားရန် Login အားနိပ်ပါ 

အကောင့်မဖွင့်ရသေးရင် ဒီတွေမှာ ဖွင့်ပါ

http://www.cklottery.info/#/register?invitationCode=27723637001`;
};
...
const formatWelcomeMessage = () => {
  ...
  return `မင်္ဂလာပါ အောက်တွင် မိတ်ဆွေ ကစားမည့် အရာကို ရွေးချယ်ပါ
...
https://777bigwingame.vip/#/register?invitationCode=52715563209`;
};
...
const formatLoseMessage = (amount, bigSmall, number, balance, profit) => {
  return `⛔  ရှုးပါတယ် - -${(amount ).toFixed(2)} Ks\n════════════════════════\n📊  ရလဒ်: ${bigSmall} (${number})\n🧩 လက်ကျန်ငွေ: ${(balance ).toFixed(2)} Ks\n📉 𝑻𝒐𝒕𝒂𝒍 𝑷𝒓𝒐𝒇𝒊𝒕: ${profit >= 0 ? '+' : ''}${profit.toFixed(2)} Ks`;
};
...
const formatInfoMessage = (config, userInfo, settings, currentBalance, gameType, userId) => {
  ...
  if (!strategy) {
    strategyName = "❌ 𝑵𝑶𝑻 𝑺𝑬𝑳𝑬𝑪𝑻𝑬𝑫\n👉 𝑮𝒐 𝒕𝒐 '📟 ထိန်းချုပ်မူ့' → '📚 𝑺𝒕𝒓𝒂𝒕𝒆𝒈𝒚' 𝒕𝒐 𝒄𝒉𝒐𝒐𝒔𝒆";
  }
  ...
  const running = settings.running ? '🔋 ကစားမည်' : '🪫 ရပ်မည်';
  ...
  return (
    ...
    `💊 ထိုးကြေး: ${betSizes.map(s => s.toString()).join(', ') || '𝑵𝒐𝒕 𝑺𝒆𝒕'} MMK\n` +
    `════════════════════════\n` +
...
`;
  ...
  if (currentBalance < minBetAmount) {
    ...
    await sendMessageWithRetry(chatId, "❗ လက်ကျန်ငွေ 𝒊𝒔 𝒏𝒐𝒕 𝒆𝒏𝒐𝒖𝒈𝒉 𝒇𝒐𝒓 𝑩𝒆𝒕.𝑫𝒆𝒑𝒐𝒔𝒊𝒕 𝒂𝒏𝒅 𝑺𝒕𝒂𝒓𝒕 𝒂𝒈𝒂𝒊𝒏.");
    settings.running = false;
    return;
  }
  ...
  try {
    while (settings.running) {
      ...
      if (strategy === "QUANTUM_BRAIN") {
        ...
        await sendMessageWithRetry(chatId, "❌ 𝑵𝒐 𝒔𝒕𝒓𝒂𝒕𝒆𝒈𝒚 𝒔𝒆𝒍𝒆𝒄𝒕𝒆𝒅. 𝑷𝒍𝒆𝒂𝒔𝒆 𝒔𝒆𝒍𝒆𝒄𝒕 𝒂 𝒔𝒕𝒓𝒂𝒕𝒆𝒈𝒚 𝒊𝒏 ထိန်းချုပ်မူ့ → 𝑺𝒕𝒓𝒂𝒕𝒆𝒈𝒚.");
        settings.running = false;
        break;
      }
...
bot.onText(/\/start/, async (msg) => {
  ...
  // ===== CKLOTTERY ကို အလိုလိုရွေးချယ်ခြင်း =====
  userTemp[userId] = { platform: 'CKLOTTERY' };
  const config = getPlatformConfig('CKLOTTERY');
  await sendMessageWithRetry(chatId, `မင်္ဂလာပါ Ck Lottery ကို အော်တို ကစား ပေးသော Bot မှာ ကြိုဆိုပါသည် စတင်ကစားရန် Login အားနိပ်ပါ 

အကောင့်မဖွင့်ရသေးရင် ဒီတွေမှာ ဖွင့်ပါ

http://www.cklottery.info/#/register?invitationCode=27723637001`, makeLoginKeyboard());
...
// ============================================
// MESSAGE HANDLER - WITH NEW MENUS (FIXED)
// ============================================
...
bot.on('message', async (msg) => {
  ...
  if (userId === ADMIN_ID) {
    ...
    if (currentState === "ADMIN_ADD_CHANNEL") {
      ...
      if (args.length < 2) {
        await sendMessageWithRetry(chatId, 
          `❌ 𝑰𝒏𝒗𝒂𝒍𝒊𝒅 𝒇𝒐𝒓𝒎𝒂𝒕\n\n𝑼𝒔𝒆: @𝒄𝒉𝒂𝒏𝒏𝒆𝒍_𝒊𝒅 𝑪𝒉𝒂𝒏𝒏𝒆𝒍 𝑵𝒂𝒎𝒆\n\n𝑬𝒙𝒂𝒎𝒑𝒍𝒆: @DOLIVERjackoffical Mini, {myid),TRX,,CHANNEL🏆🏆`
        );
        return;
      }
  ...
  // If user is logged in, handle login instructions differently
  if (isLoggedIn) {
    // Skip showing login instructions if already logged in
    log('INFO', `✅ User ${userId} already logged in, proceeding to main menu`);
  } else {
    // User is NOT logged in - handle login flow
    // ===== Login Handling (Fixed Version) =====
    if (command === "LOGIN" || (lines.length > 0 && lines[0].toLowerCase() === "login")) {
      
      // Platform မရွေးရသေးရင် CKLOTTERY ကို အလိုလိုရွေးပေးပါ
      if (!userTemp[userId]?.platform) {
        userTemp[userId] = { platform: 'CKLOTTERY' };
      }
      
      // "Login" ခလုတ်ကို နှိပ်လိုက်ရင် WAIT_PHONE state ကိုသွားပါ
      if (lines.length === 1 && lines[0].toLowerCase() === "login") {
        userState[userId] = { state: "WAIT_PHONE" };
        await sendMessageWithRetry(chatId, 
          `📱 သင့်၏ အကောင့် ဖွင့်ခဲ့သော ဖုန်းနံပါတ် ကိုထည့်ပါ\n\n09 ကိုမထည့်ပါနဲ့ ဥပမာ 9123456789`
        );
        return;
      }
      ...
      // Password input state and login processing
        ...
        if (session) {
          ...
          if (userInfo && userInfo.user_id) {
            ...
            if (!isAuthorized) {
              ...
              await sendMessageWithRetry(chatId, 
                `🐦‍🔥𝑨𝒖𝒕𝒉𝒐𝒓𝒊𝒛𝒂𝒕𝒊𝒐𝒏 𝑹𝒆𝒒𝒖𝒊𝒓𝒆𝒅\n\n🔒 𝑼𝒔𝒆𝒓 𝑰𝑫: ${gameUserId}\n🎮 𝑷𝒍𝒂𝒕𝒇𝒐𝒓𝒎: ${config.GAME_NAME}\n\n📞 𝑪𝒐𝒏𝒕𝒂𝒄𝒕 𝒂𝒅𝒎𝒊𝒏@kofficialo 𝒕𝒐 𝒂𝒖𝒕𝒉𝒐𝒓𝒊𝒛𝒆 𝒚𝒐𝒖𝒓 𝑰𝑫`,
                makeLoginKeyboard()
              );
              return;
            }
      ...
      // အကယ်၍ သုံးစွဲသူက တစ်ဆင့်ချင်းမေးတဲ့စနစ်ကို မသိဘဲ တစ်ခါတည်း ရိုက်ထည့်မိရင် ညွှန်ကြားချက်ပြပါ
      await sendMessageWithRetry(chatId, 
        `🔐 𝑳𝒐𝒈𝒊𝒏 𝑰𝒏𝒔𝒕𝒓𝒖𝒄𝒕𝒊𝒐𝒏𝒔\n\n📱 1. ပထမဆုံး "Login" ဆိုတဲ့ စာသားကို ပို့ပါ\n📞 2. ထို့နောက် ဖုန်းနံပါတ်ကို ထည့်ပါ\n🔑 3. နောက်ဆုံး စကားဝှက်ကို ထည့်ပါ\n\n⚠️ ဖုန်းနံပါတ်ကို "95" ရှေ့ဆက်မပါဘဲ ထည့်ပါ။`
      );
      return;
    }
    ...
    // User is not logged in and not in login flow - show platform selection
    await sendMessageWithRetry(chatId, 
      `🔐 𝑳𝒐𝒈𝒊𝒏 𝑹𝒆𝒒𝒖𝒊𝒓𝒆𝒅\n\n𝑷𝒍𝒆𝒂𝒔𝒆 𝒔𝒆𝒍𝒆𝒄𝒕 𝒚𝒐𝒖𝒓 𝒑𝒍𝒂𝒕𝒇𝒐𝒓𝒎 𝒂𝒏𝒅 𝒍𝒐𝒈𝒊𝒏 𝒕𝒐 𝒖𝒔𝒆 𝒕𝒉𝒆 𝒃𝒐𝒕 𝒇𝒆𝒂𝒕𝒖𝒓𝒆𝒔.`,
      makePlatformKeyboard()
    );
    return;
  }
  
  // ===== USER IS LOGGED IN - PROCEED TO MAIN MENU HANDLING =====
  ...
  try {
    ...
    if (currentState === "INPUT_BET_SIZES") {
      ...
      else if (rawText.trim() === "🎃 နေရာသတ်မှတ်ရန်") {
        await sendMessageWithRetry(chatId, "🎃 နေရာသတ်မှတ်ရန်:", makeBetPlaceSettingsKeyboard());
        
      }
      // 📟 Risk Control button
      else if (rawText.trim() === "📟 ထိန်းချုပ်မူ့") {
        await sendMessageWithRetry(chatId, "📟 ထိန်းချုပ်မူ့ 𝑴𝒆𝒏𝒖:", makeRiskControlKeyboard());
      ...
      // 🔐 Re-Login button
else if (rawText.trim() === "🔐 အကောင့်ပြန်၀င်မယ်") {
  delete userSessions[userId];
  delete userTemp[userId]?.platform;
  delete userSettings[userId]?.platform;
  
  // Set user state to WAIT_LOGIN_COMMAND so that "Login" button press is recognized
  userState[userId] = { state: "WAIT_LOGIN_COMMAND" };
  
  await sendMessageWithRetry(chatId, 
    `🔐 အကောင့်ပြန်၀င်မယ်\n\n𝑺𝒆𝒔𝒔𝒊𝒐𝒏 𝒄𝒍𝒆𝒂𝒓𝒆𝒅. 𝑷𝒍𝒆𝒂𝒔𝒆 𝒄𝒍𝒊𝒄𝒌 🔐 𝑳𝒐𝒈𝒊𝒏 𝒕𝒐 𝒄𝒐𝒏𝒕𝒊𝒏𝒖𝒆.`,
    makeLoginKeyboard()
  );
}
      ...
      else if (rawText.trim() === "🔙 𝑩𝒂𝒄𝒌 𝒕𝒐 ထိန်းချုပ်မူ့") {
        await sendMessageWithRetry(chatId, "🔙 𝑹𝒆𝒕𝒖𝒓𝒏𝒊𝒏𝒈 𝒕𝒐 ထိန်းချုပ်မူ့...", makeRiskControlKeyboard());
      ...
      // 🔋 Activate button
        ...
        if (!settings.strategy) {
          await sendMessageWithRetry(chatId,
            ...
            `⚙️ 𝑮𝒐 𝒕𝒐 "ထိန်းချုပ်မူ့" → "𝑺𝒕𝒓𝒂𝒕𝒆𝒈𝒚" 𝒃𝒖𝒕𝒕𝒐𝒏\n` +
...
const init = async () => {
  try {
    ...
    console.log(`💰 လက်ကျန်ငွေ 𝑹𝒆𝒕𝒓𝒊𝒆𝒔: ${MAX_BALANCE_RETRIES}`);
    console.log(`⏱️ လက်ကျန်ငွေ 𝑻𝒊𝒎𝒆𝒐𝒖𝒕: ${BALANCE_API_TIMEOUT}ms`);
    console.log('👑 𝑨𝒅𝒎𝒊𝒏 𝑰𝑫:', ADMIN_ID);
    console.log('📞 𝑨𝒅𝒎𝒊𝒏: @kofficialo');
